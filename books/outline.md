...
## 10. 認可コードを不正に取得する攻撃と防御 (confidential client)

- 概要
  - 認可コードを不正に取得する攻撃手法とその防御方法について解説することを明示
  - confidential client で発生しうる攻撃手法に限定することを再度明示
- リダイレクト URI 攻撃
  - 攻撃の概要
    - リダイレクト URI の値を書き換えることで
    - 攻撃者が用意したリダイレクト先に認可コードを送らせる
    - 認可コードを奪取する攻撃手法であることを説明
    - 一般的な Web セキュリティでたとえるとオープンリダイレクト攻撃に相当する
    - 攻撃の流れを提示
      - (省略)
  - 攻撃の対策
    ...
    - この対策によって、攻撃を防止できることを流れで提示
      - (省略)
- まとめ
  - 認可コードを奪取する攻撃の例とその防御方法を理解した
- 余談
  - public client では他にも認可コードを奪取する攻撃手法が存在することに言及

## 11. 認可コードを用いてアクセストークンを不正に取得する攻撃と防御 (confidential client)

- 概要
  - 認可コードを用いてアクセストークンを不正に取得する攻撃手法とその防御方法について解説することを明示
  - confidential client に限定することを再度明示
- 認可コードインジェクション攻撃
  - 攻撃の概要
    ...
    - 攻撃の流れを提示
      - (省略)
  - 攻撃の対策
    - PKCE (Proof Key for Code Exchange) による防御方法を説明
      - リソースオーナーに認可コードを取得させるよう指示したクライアントと
      - アクセストークンを取得しようとするクライアントが同一であることを検証する仕組みであることを説明
    - OAuth フロー開始時の段階で
      ...
      - この方式で何故攻撃を防止できるか？
        ...
        - 認可コードをすり替えるというケースにおいて、実質的に別クライアントの認可コードを流用することになるため
...
- まとめ
  ...
  - PKCE を導入することで、認可コードインジェクション攻撃を防止できることを強調
- 余談
