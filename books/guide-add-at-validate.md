...
- 両者に共通する検証手順
  ...
  - 検証において必要なメタデータはおおよそ次の通り
    ...
    - 受信者 (audience)
      ...
      - 余談: 配列の場合もある
        - 複数の RS を想定している場合など
        - その場合、RS は自分の識別子が配列内に含まれていることを確認する
    - スコープ (scope)
      - AT に紐づくスコープ情報
      - RS はリクエストされた操作がスコープ内か確認する
      - スコープの検証はアプリケーションの実装に依存するため
      - 具体的な方法はここでは解説しない
  - トークン検証時(特に Handle 式) に明確な仕様が決まっている訳ではないため、
  - 実装ごとに異なる場合がある
  - ここでは多くの実装を踏まえた一般的な流れを解説する
    ...
    - 以下の内容を自分で検証する
      - AT が有効かどうか
        - AT の有効性自体は AS が判断する
          - AT が正当に発行されたかどうか
          - AT が有効期限内かどうか
        - したがって、Handle 式 の AT 有効性検証ステップ は、
        - RS は AS の応答を確認するだけで良い
