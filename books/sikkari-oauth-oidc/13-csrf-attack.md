...
つまり、攻撃者が被害者に **リソースを押し付ける** 形になります。

![](/images/12-csrf-attack/2025-12-02-18-34-28.png)
...
![](/images/12-csrf-attack/2025-12-02-18-48-29.png)
...
![](/images/12-csrf-attack/2025-12-02-18-56-47.png)

### state パラメータとは

そして state パラメータは、
**リソースオーナーに認可コードを取得させるよう指示したユーザセッション**と
**認可コードを受け取ったユーザセッション**に一貫性があることを、
クライアント自身が検証するための仕組みです。

![](/images/12-csrf-attack/2025-12-03-01-30-17.png)

state パラメータ自体は、クライアントが任意に設定でき、推測困難なランダム文字列を利用します。
検証の主体は **クライアント** であることに留意してください。

![](/images/12-csrf-attack/2025-12-02-19-21-44.png)
...
1. **フロー開始時**
   ...
   - リソースオーナーにフロー開始の指示をする際、state を付与
   - 認可リクエスト時に state を認可サーバに渡す

1. **認可レスポンス時**

   - リソースオーナー → 認可サーバ
     認可サーバ → リソースオーナー において state はそのまま伝搬される

1. **認可コード受け取り時**
   - クライアントはセッションから state を取り出す
   - リソースオーナーから渡された state と比較検証
   - 一致すればフローを継続、不一致ならフローを中断

クライアントが、state をセッションに紐づく形で保持するのがポイントです。

![](/images/12-csrf-attack/2025-12-02-19-34-05.png)
...
![](/images/12-csrf-attack/2025-12-02-23-29-11.png)
...
1. サーバがランダムな CSRF トークンを生成し、セッションに保存
2. フォームの hidden フィールドに CSRF トークンを埋め込む
3. フォーム内容を受け取ったサーバが CSRF トークンの一致を検証
4. 一致しなければリクエストを拒否
...
3. 認可コードと state を受け取ったクライアントが state の一致を検証
4. 一致しなければフローを中断
5. どちらも**ユーザセッションの一貫性を検証**して CSRF 攻撃を防ぐ点で共通しています。

![](/images/12-csrf-attack/2025-12-02-23-45-44.png)

### state パラメータと CSRF トークンの類似点

では、state パラメータと CSRF トークンの違いを整理してみましょう。

| 項目           | CSRF トークン                                                                      | state パラメータ                                                                         |
| -------------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| 生成タイミング | フォーム表示時                                                                     | フロー開始時                                                                             |
| 保存先         | セッション                                                                         | セッション                                                                               |
| 検証タイミング | フォーム内容受取時                                                                 | 認可コード取得時                                                                         |
| 検証内容       | フォームにアクセスしたユーザセッションと<br>フォーム送信したユーザセッションの一致 | フローを開始しようとしたユーザセッションと<br>認可コードを返却したユーザセッションの一致 |

state パラメータは、**OAuth フローにおける CSRF トークン**であると言えます。[^rfc6749-state]

[^rfc6749-state]: RFC 6749 Section 4.1.1 において、state パラメータは CSRF 対策のために使用され、推測不可能な値を使用してリクエストとコールバックを紐づけることが推奨されています。https://www.rfc-editor.org/rfc/rfc6749.html#section-4.1.1
...
| 項目             | PKCE                                                                 | state                                                                        |
| ---------------- | -------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| 用途             | フローにおけるクライアントの一貫性を検証                             | フローにおけるユーザセッションの一貫性を検証                                 |
| 生成する登場人物 | クライアント                                                         | クライアント                                                                 |
| 検証する登場人物 | **認可サーバ**                                                       | クライアント                                                                 |
| 一致検証する対象 | フローを開始したクライアント<br>＝トークンリクエストしたクライアント | フローを開始した際のユーザセッション<br>＝認可コードを渡したユーザセッション |
