...
前章 [21 章 OAuth まとめと OAuth 2.1 の紹介](/calloc134/books/sikkari-oauth-oidc/viewer/21-oauth-summary)で学んだ通り、OAuth 2.0 は**認可**のためのフレームワークです。
...
![](/images/21-oidc-concepts/2025-12-28-15-54-39.png)

## ID トークンの解説

### ID トークンとは

**ID トークン** は、ユーザの**認証結果の証明書**としてのトークンです。

OAuth 2.0 の認可コードフローではアクセストークンを用いてリソースにアクセスしましたが、
OIDC ではそれに加えて ID トークンが発行されます。
ID トークンには**認証結果・認証ユーザを対象とした情報** が含まれます。
なお、認証結果・認証ユーザを主体としたとき、それに関する属性情報を **クレーム** と呼びます。

![](/images/21-oidc-concepts/2025-12-28-16-11-11.png)

### OIDC における ID トークンの役割

OIDC では、以下のような流れでユーザ認証が行われます。
...
ここで重要なのは、OIDC の目的が **ユーザ認証** であるという点です。
...
**この鍵があればリソースにアクセスできてしまう**ため、
漏洩には細心の注意が必要です。

クライアントはアクセストークンを保持しますが、
アクセストークンを検証するのはリソースサーバーです。
**クライアントはアクセストークンを検証しない**ことに注意しましょう。
...
長期間の保持は不要です。
ユーザが確かに認証されたことを検証し、セッションを開始したら、
...
| 保持期間 | 長期（リフレッシュ） | 短期（検証後は不要なことも） |

![](/images/21-oidc-concepts/2025-12-28-16-22-22.png)
