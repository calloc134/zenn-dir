...
前章 [6 章 アクセストークンの形式](/calloc134/books/sikkari-oauth-oidc/viewer/06-access-token-format)ではアクセストークンの形式（Handle 式 / Assertion 式）について解説しました。
...
## クライアントからリソースサーバへのアクセストークン送付

リソースサーバでの検証手順を解説する前に、
クライアントがリソースサーバにアクセストークンを送付する手順について確認します。

クライアントは、リソースサーバの保護されたリソースにアクセスする際、
HTTP リクエストの `Authorization` ヘッダーにアクセストークンを含めて送信します。

一般的には、`Bearer` スキームを使用してアクセストークンを送付します。
