...
インプリシットフローは、
認可コードを介さず、
リソースオーナーのデバイスが直接**認可レスポンスで直接アクセストークンを受け取り**、
それをクライアントに渡すフローです。

![](/images/19-implicit-flow/2025-12-25-15-40-44.png)
...
| -------------------- | ------------------------------------------------------------------------------------- | ---------------------------------------- |
| 認可コードフロー     | 認可リクエスト <br>→ 認可レスポンス <br>→ トークンリクエスト <br>→ トークンレスポンス | バックチャネル<br>（サーバ間通信）       |
| インプリシットフロー | 認可リクエスト <br>→ 認可レスポンス（トークン付き）                                   | **フロントチャネル<br>（ブラウザ経由）** |

認可コードフローは、サーバ間通信でアクセストークンを受け渡します。
デバイス側にアクセストークンが露出しない、バックチャネルでの受け渡しです。

![](/images/19-implicit-flow/2025-12-25-15-56-51.png)

一方インプリシットフローは、認可レスポンスで直接アクセストークンを受け取り、ブラウザを経由してクライアントに渡します。
**アクセストークンがブラウザに露出** する、フロントチャネルでの受け渡しです。

![](/images/19-implicit-flow/2025-12-25-16-00-29.png)
...
攻撃者のアクションの回数が減ることで、**攻撃のハードルが低くなって**しまいます。

![](/images/19-implicit-flow/2025-12-25-16-14-50.png)
...
![](/images/19-implicit-flow/2025-12-25-16-24-46.png)
...
## 余談：OAuth を外部ログインに転用した場合の危険性

インプリシットフローの危険性は、OAuth を外部ログインに転用した場合にさらに拡大します。

詳細は OIDC の章で説明しますが、インプリシットフローでアクセストークンを外部ログインに使用すると、**アクセストークンの差し替え攻撃**が可能になります。
