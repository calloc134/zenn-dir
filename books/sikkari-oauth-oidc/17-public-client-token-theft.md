...
この攻撃は、
攻撃者が事前に奪取した認可コードを用いて**クライアント認証のない Public Client になりすまし**
...
となります。
そのため、攻撃者が認可コードを奪取できれば、簡単にアクセストークンを取得できてしまいます。
...
- Public Client ではクライアント認証ができないため、
  認可コードが奪取されるとアクセストークンも取得されてしまう
- **PKCE** がこの問題を解決する
...
| PKCE                       | 推奨                       | **必須**      |
| クライアント偽装攻撃の防御 | クライアント認証で防御可能 | PKCE で防御   |
