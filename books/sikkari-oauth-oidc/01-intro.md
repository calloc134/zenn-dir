...
本書は、OAuth 2.0 / OAuth 2.1 と OpenID Connect (OIDC) の基礎をしっかりと学ぶことを目的としています。

「OAuth って聞いたことはあるけど、いまいちよくわからない」「OIDC を使っているけど、仕組みを説明しろと言われると困る」という方に向けて、OAuth/OIDC の基本概念、フロー、セキュリティ対策などを丁寧に解説していきます。

OAuth/OIDC をなんとなく理解している状態から、**しっかり理解している**状態へステップアップすることを目指しましょう。
...
:::message
IDaaS（Auth0、Firebase Authentication など）の実装はベンダーによって独自性が強く、その認識が学習の妨げとなる場合もあります。本書を読む際は、既存の IDaaS の仕様を一旦忘れることをおすすめします。
:::
