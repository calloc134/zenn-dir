...
OAuth 2.0 の仕様である RFC 6749 では、以下のように説明されています。

> 例えば, あるユーザ (リソースオーナー) が, 印刷サービス (クライアント) に対して, 写真共有サービス上 (リソースサーバー) に保管されている彼女の保護された写真へのアクセス権を与えることを考える. OAuth では, その際彼女のユーザ名とパスワードを印刷サービスに与える必要はない. そのかわり, 彼女は写真共有サービスの信任を得たサービス (認可サーバー) に対して認証を行い, 印刷サービスへのアクセス権限委譲用クレデンシャル (アクセストークン) を発行させる
>
...
| 役割                                   | 名称             | 役割の説明                                                                   |
| -------------------------------------- | ---------------- | ---------------------------------------------------------------------------- |
| リソースの持ち主                       | リソースオーナー | リソースの持ち主。リソースへのアクセスを許可する主体                         |
| リソースを管理している保管庫           | リソースサーバ   | リソースを管理している保管庫。アクセストークンを検証し、リソースを提供する   |
| 保管庫へのアクセスを管理している認可局 | 認可サーバ       | リソースサーバへのアクセスを管理する。アクセストークンを発行する             |
| リソースにアクセスする第三者           | クライアント     | リソースにアクセスする第三者。アクセストークンを用いてリソースにアクセスする |
