...
認証情報の渡し方には おおよそ 2 つの方式があります。

| 方式                  | 説明                 |
| --------------------- | -------------------- |
| `client_secret_basic` | HTTP Basic 認証方式  |
| `client_secret_post`  | リクエストボディ方式 |
...
このように、クライアント認証を行うことで、
認可サーバはトークン引き換えリクエストを送信してきたクライアントが正当なものであることを確認できます。
...
OAuth 2.0 では、`client_secret_post` は非推奨の扱いになっていました。
