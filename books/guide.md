...
- 概要
  ...
  - クライアント
    ...
    - リソースオーナーから許可を得て、認可局からアクセストークンを取得し、保管庫にアクセスする

  - リソースサーバ
    - リソースを管理している保管庫
    - Google Photos API など、リソースオーナーのリソースを保管・提供するサーバ
    - アクセストークンを検証し、正当なクライアントに対してリソースを提供する

  - 認可サーバ
    - 保管庫へのアクセスを管理している認可局
    - Google OAuth 2.0 認可サーバなど、クライアントからの認可リクエストを受け付け、アクセストークンを発行する
