...
- OIDC の主要なロール
  ...
  - リソースサーバ -> UserInfo Endpoint を提供するサーバ
    ...
    - 仕様では OpenID Provider と同一のサーバでもよいとされている
    - そのため、OpenID Provider と同じサーバが提供する場合も多い
    - また、ID トークンで十分な場合はあまり利用されない傾向にある
- OIDC 内に存在する認証らしさと認可らしさ
  - ID トークン ⇒ OIDC で追加された認証の特性が強い
  - /userinfo ⇒ OAuth 2.0 の機構を引き継いだ認可の特性が強い
- 認可コードフロー
...
- 構成要素
  ...
  - ペイロード部分
    ...
    - JWT ではペイロードに JSON 形式のデータを含められる
      - JSON 形式のデータ = JSON Claims Set と呼ばれる
    - JWT の場合、登録済み
