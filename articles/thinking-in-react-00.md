...
**それによりどのようなことを実現しようとしているのか** を解説していきます。
...
また、今回は複雑さを避けるため、Svelte や Solid.js のような
コンパイルベースの宣言的 UI フレームワークについての言及を割愛させていただきます。
申し訳ございません。
:::
...
ここでいう「過去の状態」とは、UI フレームワークによって様々です。

例えば React では、
要素を元に生成した内部状態用のオブジェクト (Fiberツリー) です。
Vue.js では 過去の要素 (VNode) をそのまま過去の状態として利用します。
...
開発者は「この場合、理想の見た目はこういう見た目」だけを宣言すればよく、
「前の見た目からどう変更するか」という差分の計算はフレームワークが担当してくれます。

## 差分検知アルゴリズム: Reconciliation

差分検知アルゴリズムは **Reconciliation** と呼ばれます。

Reconciliation とは フロントエンド領域 のみに存在する単語ではなく、
宣言的なシステムを構築する際に広く用いられる概念です。

例えば、インフラ、特にコンテナを宣言的に管理できるソフトウェアとして
Kubernetes というものが存在します。

この Kubernetes の内部でも、 Reconciliation Loop という仕組みを用いて
宣言的な コンテナ・インフラ管理を実現しています。

**「あるべき姿」と「現在の姿」を比較し、差分を埋める** という考え方は、
...
2. **React が内部的に保持する内部状態**
   - Fiber ノード もしくは Fiber ツリー（仮想的に状態を表現したデータ構造）

ここからは それぞれ **「React 要素」** と **「Fiber ノード (Fiber ツリー)」** と呼び、
これら2つを明確に区別して説明していきます。

## 要点まとめ

- 宣言的 UI の本質は、**How ではなく What を記述する**こと
- フレームワークは **過去の状態と今回の要素を比較**して差分更新する
- 開発者の主な責務は **「理想の見た目」だけを宣言すること**
- 本シリーズでは「仮想 DOM」を曖昧に使わず、<br />**React 要素と Fiber ノード (ツリー) を分けて捉える**
