...
topics: ["react", "typescript", "tanstackrouter", "型安全"]
published: true
---
...
`routeTree` とは、ルーティングのツリー構造を表す変数です。
これを `createRouter` に渡すことで、ルーティングを行うための `router` が作成されます。

ルーティングを行うコンポーネントとして、`RouterProvider` を利用します。
これを利用することで、ルーティングが有効になります。
...
ここで、`createRoute` には、`getParentRoute` と `path` を指定します。
...
この使い方は、`React Router`と似たような点が多いです。
...
ネストされたルートでは、基本的に親のルートで外枠のコンポーネントを指定し、その中で`<Outlet>`コンポーネントを配置することでそこに子のルートのコンポーネントがマッピングされる形として実装します。
