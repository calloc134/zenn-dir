...
まず、意義的な説明をします。
今回のアプリケーションでは、Auth0 を用いた外部ログインを実装しています。
外部ログインとは、認証の仕組みです。

今まではバックエンド API をリソースとして解釈し、
それを認可するために OAuth のアクセストークンを利用していました。

しかし構成の変更により、バックエンド API をリソースとして認可する必要がなくなり、
**フローにおいてリソース認可が発生しなくなった**のです。

認可が発生しないのであれば、認可のしくみを利用する必要はなく、
認証の仕組みを用いてユーザの身分証明を行えば十分です。
したがって、OIDC の仕組みを利用し、
ID トークンを用いてユーザの身分証明を行うのが適切な設計となります。

次に、技術的な視点でも説明します。
バックエンド API は OAuth リソースサーバとして動作せず
...
- 外部ログインに OAuth 認可の仕組みではなく
  OIDC 認証の仕組みを利用しており、シンプルで健全
- バックエンドが Confidential Client として動作するため
  フロントエンドにトークンが露出せず、セキュリティリスクが低減する
- セッションクッキーを HttpOnly 属性付きで発行でき
  悪意あるスクリプトからの盗難リスクが低減する
...
:::message
`@auth0/auth0-hono`は、
クッキーの上に JWE 形式のセッション情報を載せる実装を提供しています。
この実装は、セッション管理の一種です。
