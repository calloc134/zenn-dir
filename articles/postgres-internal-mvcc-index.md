...
トランザクション実現のためには、データベースの状態をある瞬間で固定し
整合性を保つための仕組みが必要です。
その仕組みが、先程解説した MVCC およびスナップショットです。
...
Repeatable Read とは、
A トランザクション毎に、つまり
トランザクションの最初の文が開始したタイミングでスナップショットを取り、
データベース状態の瞬間を切り抜くトランザクション分離モデルです。
...
Repeatable Read ではトランザクション開始後に取得したスナップショットが使われ続けるため、
データベースの状態が固定され、変更が反映されなくなるのです。
詳細の実装は後述します。
...
- トランザクションの可視性チェックアルゴリズム
- 物理タプルの可視性チェックアルゴリズム

の 2 段階で構成されています。
そして物理タプルの可視性チェックアルゴリズム内部で、
トランザクションの可視性チェックアルゴリズムを呼び出す形で動作します。
...
の判定を行うことで、トランザクションの可視性をチェックできます。
しかし、この方法だとすべてのトランザクションについて判定を行う必要があり、非効率です。

ここで、スナップショットの `xmin`/`xmax` パラメータが登場します。
...
これらを踏まえて、
トランザクションの可視性チェックアルゴリズムは以下のように動作します。
...
このようにして、トランザクションの可視性チェックアルゴリズムは動作します。
...
先程解説したトランザクションの可視性チェックアルゴリズムが呼び出される形で動作します。
...
|                          | 挿入(A) が可視 | 挿入(A) が不可視 |
| ------------------------ | -------------- | ---------------- |
| 削除 or 更新(B) が可視   | 不可視         | 不可視           |
| 削除 or 更新(B) が不可視 | 可視           | 不可視           |
...
3. `t_xmin` のトランザクションが自分のトランザクションでないなら
   1. トランザクションの可視性チェックアルゴリズムを用いて `t_xmin` の可視性をチェック
...
![](/images/postgres-internal-mvcc-index/2026-01-04-15-18-01.png)
...
3. `t_xmax` のトランザクションが自分のトランザクションでないなら
   1. トランザクションの可視性チェックアルゴリズムを用いて `t_xmax` の可視性をチェック
...
![](/images/postgres-internal-mvcc-index/2026-01-04-15-28-02.png)

このようにして、物理タプルの可視性チェックアルゴリズムは動作します。

A/B において、ステップ 2、ステップ 3 のアルゴリズムは、処理の内容が同じですが、
可視・不可視の判定が逆になる点に注意してください。
