...
この記事を通して、PostgreSQL の内部実装の理解が深まれば幸いです。
...
あるスナップショットに基づく読み取り操作は、そのスナップショットが取得された瞬間のデータベースの状態が固定されているかのように振る舞います。
...
**スナップショットを取るタイミングが「文ごと」か「トランザクションごと」か**であることに起因します。
