...
- CORSポリシーとは
  - クロスオリジンリクエストにおける ブラウザの挙動を制御・制限する仕組み
    - unsafe methodの場合
      ...
      - 注意点
        - unsafe method の場合でも
        - 特定の条件を満たす場合、safe method と同じ挙動となることがある
        - 条件は後述
    - safe methodの場合
      - レスポンスの閲覧可否 を制御・制限する
  ...
  - CORS ポリシーが有効な場合の挙動
    ...
    - unsafe method であっても
      - 以下の条件を満たす場合は safe method と同じ挙動になる
        - POST メソッドである
        - リクエストヘッダが 以下のいずれかである
          - Accept
          - Accept-Language
          - Content-Language
          - Content-Type (ただし 以下のいずれかに限る)
            - application/x-www-form-urlencoded
            - multipart/form-data
            - text/plain
        - カスタムヘッダを含まない
        - リクエストボディが Blob, BufferSource, FormData, URLSearchParams, USVString, または null である
