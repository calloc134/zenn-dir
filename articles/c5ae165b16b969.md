---
title: "大学の授業でCloudflare使いまくって自己満足開発した"
emoji: "🔥"
...
> 大学の授業で"出席管理システム"を作ることになりました
> せっかくなので Cloudflare のインフラで最新技術を使いまくりました
> 楽しかったです
...
この授業では、生徒が自分たちで解決したいテーマを決め、それに沿って作品を開発していく実践的な授業です(授業というより、ハッカソンに近い感じの演習です)。
...
今回は Bluetooth 検知システムも最低限実装しましたが、この記事では出席管理システムのみについて記述します。
...
今回技術選定するにあたって、是非 Cloudflare Workers と Cloudflare Pages を使いたいと思いました。
...
![SQLエアプでごめん](/images/c5ae165b16b969/2023-12-20-18-24-57.png)
...
これは、教師が授業を作成するときに、授業用テーブルにレコードを作成することで実現できます。

#### 生徒がその ID を参照して、授業に履修する

これは、生徒が授業に履修するときに、履修用テーブルにレコードを作成することで実現できます。
...
授業に履修している かつ 出席が完了していない生徒の生徒 ID & デバイス ID を取得する、という処理を、一度に行えばいいのかも。
:::
...
(というかセキュリティ考えるのであれば Authorization ヘッダに JWT を入れるのではなく、Cookie に JWT を入れるべきだったかもしれない　まあいいか・・・)
...
:::message
(書いてて思ったんですが 行儀のよい書き方なのか不安です うーん)
...
スキーマを統一してバリデーションの実装をフロントバック統一したい・・・ぐぬぬ
...
悪意のある生徒が、出席管理センサからのデータに偽装して出席を行うことができてしまいます。これは大変だ

以後、JWT を使って認証を追加したいです。

### CSS が汚い

copilot をふんだんに利用してデザインを作成したのですが、やたら marginBottom を利用したコードになっています。
脱 margin して padding と gap を使うようにしたいなあ・・・のお気持ちです。

## おわりに

何はともあれ、無事一人でそれっぽい出席管理システムを構築できて、とてもよかったです！
