...
初めに、今回の解説では以下のような内容をスキップしましたことをおことわりしておきます。
...
React はタスクをキューに登録する際に、優先度に応じてタイムアウト値というものを計算します。これはミリ秒で表現され、最大遅延時間とも表現されます。要するにタスクをどれだけ後回しにしていいかという値です。
また、タスクの期限切れ時刻も計算します。これは開始予定時刻 + タイムアウト値で計算されます。
