---
title: "再考React - 第1回 Reactのコア思想である純粋関数性"
emoji: "⚛️"
...
React は関数コンポーネントが純粋関数であることを要求しています。
...
UI を記述する React 要素を生成する関数コンポーネントを
`f(...) = React 要素` という純粋関数性を保つように設計する必要がある、というのは
**React の真髄とも言える重要な設計思想** です。
...
開発者に対し、**責務の混在しないコードを書くことを強制** します。
...
### 2. 純粋関数性を前提とした React 機能の活用

Reactが関数コンポーネントに純粋関数性を要求する理由の 2 つ目は、
**React の提供する機能 を不具合なく利用できるようにするため** です。

Reactは、関数コンポーネントが純粋関数性を持つことを前提としています。

この前提を置くことで、React の提供する機能を最大限に活用することができます。

- 関数コンポーネント実行の一時停止・再開による ユーザ操作への応答性向上
- 関数コンポーネント実行の中断 (サスペンド・Suspense)
- パフォーマンスの最適化（React Compiler によるメモ化の自動化など）

これは React 自体の性能向上やユーザ体験の向上に寄与します。

React は、純粋関数性を前提とした機能拡張を、
今後も積極的に増やしていく方針を示しています。

React の提供する機能の恩恵を最大限に受けるためにも、
関数コンポーネントが純粋関数性を保つように設計することが重要です。

純粋関数性を保つことは、**開発者・React 双方にとって大きな利益をもたらす**
という理解が大事です。
...
これらの処理は、**外部システムからの値の読み取り** に該当する処理といえます。
...
これらの処理は、**外部システムへの値の書き込み** に該当する処理といえます。
...
**純粋関数性** を保つことを強く推奨しています。

開発者は、関数コンポーネントの実装において
副作用を避けるように努める必要があります。
...
この呼び出しは、おおむね**状態群のいずれかの値が変化したとき** に行われます。

入力値である状態群のいずれかの値が変化した時に、
React は関数コンポーネントを再実行し、React 要素を生成するという流れになります。
...
これにより、**状態変化に応じた React 要素、**
つまり**見た目の更新** を実現することができています。

関数コンポーネントを関数としてみなして考えると、
**入力が変化した際、それに反応して出力（React 要素）を再生成する** という
設計になっているといえます。

:::message

厳密に言えば、
「値が変化したときに関数コンポーネントが再実行」という表現は
props の場合には当てはまりません。

props が登場するということは、
親コンポーネントの下に子コンポーネントが存在するという構造です。

React は、親コンポーネントが再レンダーされるタイミングで
子コンポーネントが再実行される、といった動作をします。

この動作は、props 自体の変化を考慮しない動作となります。
:::

# 入力値はスナップショット

## 状態群のイミュータブル性とスナップショット

関数コンポーネント実行時における状態群は、
関数コンポーネントの実行開始時に値が固定され、終了まで変化しない
**イミュータブル（不変）なデータ**として扱われます。

React はこの特性を、**スナップショット**という考え方で説明しています。

**スナップショット**とは、「ある時点の状態を切り取った」
イミュータブル（不変）なデータのことです。
React は、状態群をスナップショットとして扱うと説明しています。

具体的に見てみましょう。
一つのレンダーフェーズ（関数コンポーネントの実行）において、
関数コンポーネント実行開始時の状態群の値が固定され、
**終了まで変化せず一貫して同じ値が保たれる** ことが保証されます。
...
この一貫性は、**関数の純粋関数性を保つ必須要件** とも言えます。
関数コンポーネントの実行中で状態、つまり入力値が変化してしまうと、
同じ値を返せなくなってしまうおそれがあるからです。

:::message
**スナップショットとバッチ処理の関連性**

React は、状態群をスナップショットとして扱うという要件を満たすため
**状態更新のバッチ処理** を行います。

状態更新のバッチ処理とは、関数コンポーネント実行中の更新を
即時反映せず蓄えておき、後の段階でまとめて更新を処理するということです。

状態群をスナップショットとして扱う仕組み上、バッチ処理が必須です。
...
そのため、開発者がこれらの値を関数コンポーネント実行中に変更することは、
やろうと思えば可能です。

しかし React は
関数コンポーネント実行時における状態群を
「スナップショットとしてイミュータブルに扱う」という前提で動作しています。

そのため、これらの値を関数コンポーネント実行中に変更することは **React の設計思想に反し**、
**純粋関数性を失い、予期しないバグを引き起こす**可能性があります。

開発者は、状態群の値をしっかりイミュータブルに保ち、
関数コンポーネント実行中に変更しないよう、注意して実装する必要があります。
...
- 純粋関数性の狙いは **責務分離** と **React 機能の最大限の活用**
- 関数コンポーネント内での外部読み書きは副作用であり、**要素生成処理に混ぜない**こと
- state / props / context は **スナップショットとして一貫した値**として扱われる
  - 開発者も これらの値を変更しないよう注意すること
