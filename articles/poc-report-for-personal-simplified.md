...
Promise は、JavaScript で非同期処理の結果を表す object である。まだ結果が出ていない処理を Promise として受け取り、成功したら `resolve`、失敗したら `reject` される。

`async/await` は、この Promise を読みやすく扱うための構文である。`await promise` と書くと、その Promise が解決されるまで待ち、解決後の値を取り出す。
...
Promise には、`then` をチェーンできるという性質がある。
...
ここで重要なのは、`then` の callback がさらに Promise を返した場合でも、Promise はその内側の Promise を待ってから次の `then` に値を渡す点である。
...
この性質により、Promise のネストは次のようにフラットな chain として扱える。
...
この「返ってきた Promise をさらに解決して取り込む」性質があるため、Promise は非同期処理をネストではなく chain として表現できる。

ここで thenable という概念が出てくる。thenable とは、Promise そのものではなくても `then` メソッドを持つ object のことである。

JavaScript の Promise 解決処理は、値が Promise instance かどうかだけを見るのではなく、「`then` という callable property を持っているか」を見る。これは duck typing の一種である。

duck typing とは、「その object が何の class の instance か」ではなく、「必要な形や振る舞いを持っているか」で扱いを決める考え方である。たとえば「`then` を持っているなら Promise のように扱える」と判断するのが thenable の発想である。

そのため、Promise ではない plain object でも、callable な `then` を持っていれば Promise のように呼び出される。
...
この object は `Promise` ではない。しかし `then` を持つため、`await` / Promise 解決処理から Promise 的なものとして扱われる。

Flight プロトコルのデコードでは、後述する `chunk` という object が出てくる。この chunk は React Flight の内部状態を表す object だが、同時に `then` を持つため Promise 的な挙動をする。つまり、React Flight の decode 処理では「chunk が thenable として待たれる」場面がある。
