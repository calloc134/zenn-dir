...
また、`kty: oct` (対称鍵) が JWKS に含まれること自体、
セキュリティ上の問題があります。
...
特に `hono/jwt` の問題は、JWT ヘッダの `alg` を直接信用していたというより、
...
このコードを書いた利用者が、`publicKey` に RSA 公開鍵を渡して
RS256 で検証しているつもりだったとしても、
`alg` を省略していると内部では `HS256` が選ばれます。

その結果、攻撃者が同じ公開鍵を HMAC のシークレットとして使って
`HS256` の JWT を作ると、Hono 側も同じ公開鍵を `HS256` のシークレットとして扱ってしまい、
署名検証を通過できる可能性がありました。

つまり JWT ミドルウェアの問題は、
「JWT ヘッダの `alg` を全面的に信用した」というよりも、
**アルゴリズム指定が省略可能で、かつ省略時のデフォルトが `HS256` だったため、
公開鍵を HMAC シークレットとして誤用できる状態になっていた**
という点にありました。

もう一方の `hono/jwk` の問題は、より古典的なアルゴリズム混同攻撃に近い形です。
...
ここで重要なのは、JWK の `alg` フィールドが RFC 上任意であることです。
実運用の JWKS では `alg` が入っていない鍵もあり得ます。
その場合、修正前の Hono は JWT ヘッダに入っている `alg` を検証アルゴリズムとして使っていました。

攻撃者から見ると、これは次のような状態です。

1. JWKS エンドポイントから RSA 公開鍵を取得する
2. `kid` は既存の鍵に合わせる
3. JWT ヘッダの `alg` を `HS256` にする
4. 取得した RSA 公開鍵を HMAC シークレットとして署名する
5. JWK 側に `alg` がなければ、Hono は `header.alg` の `HS256` で検証してしまう

なお、`alg: none` については、
Hono の `isTokenHeader()` が対応アルゴリズムの一覧に含まれる値だけを受け付ける実装だったため、
今回の主な攻撃経路ではありませんでした。
問題の中心は、`none` による署名なし JWT ではなく、
**公開鍵として配布されている値を、HS256 の共有シークレットとして再解釈できてしまうこと**です。

また、JWK ミドルウェアでは当時、
JWKS 検証に対称鍵アルゴリズムである `HS256` / `HS384` / `HS512` を使うことを明示的に拒否していませんでした。
JWKS は基本的に公開鍵を配布する仕組みとして使われるため、
そこに HMAC のような共有シークレット方式を混ぜられる設計は危険です。

まとめると、今回見つかった Hono の脆弱性は次の 2 点でした。

- `hono/jwt`: `alg` を省略でき、省略時に `HS256` へフォールバックするため、公開鍵を HMAC シークレットとして誤用できる
- `hono/jwk`: JWK に `alg` がない場合に JWT ヘッダの `alg` を採用し、さらに HS 系アルゴリズムを拒否していなかった
