...
では、この脆弱性を更に詳しく解説する前に、JWT と JWK の基本的な構造を整理しておきます。
...
OIDCやOAuth 2.0などのプロトコルで、
公開鍵を配布するためのフォーマットとして広く利用されています。
...
ここからは、今回の脆弱性の前提となる `alg` の扱いと、アルゴリズム混同攻撃について解説します。

## 古典的なアルゴリズム混同攻撃とは

今回のHonoの前提となる、
JWT実装における古典的なアルゴリズム混同攻撃について解説します。
...
その前提として、**JWT ヘッダの `alg` フィールドを信用してはいけない**ということがあります。

RFC 7515 (JWS) Section 5.2 では、
どのアルゴリズムを利用可能とするかはアプリケーション側の判断であり、
JWS の検証に成功したとしても、そのアルゴリズムがアプリケーションにとって許容できない場合は
JWS を無効とみなすべきだとされています。

また RFC 8725 (JWT Best Current Practices) Section 3.1 では、
ライブラリは呼び出し側がサポートするアルゴリズム集合を指定できるようにし、
その集合以外のアルゴリズムを暗号操作に使ってはならないとされています。

検証側は事前に **「このシステムで許可するアルゴリズム」の集合**を定義し、
JWT ヘッダの `alg` が**その集合に含まれているかを確認**してから
JWTの検証を行う必要があります。

JWT ヘッダは攻撃者が書き換え可能な外部入力であり、
その値をそのまま信用して検証アルゴリズムを選択することは、
攻撃者に検証アルゴリズムの選択権を与えることになり、
アルゴリズム混同攻撃のリスクを生みます。

:::message

ここで、「アルゴリズムを選択できるとはいえ、
攻撃者が`alg`を書き換えるためには JWTを偽造する必要があるのでは？」
と思う方もいるかもしれません。
...
この「鍵」の役割の違いを悪用することで、
**公開されている RSA 公開鍵を「HS256 のシークレット」として使うこと**で、
有効な HS256 署名、つまり偽造 JWT を生成することが可能になります。

したがって、悪意のある攻撃者は、JWT ヘッダに `HS256` を書き込み、
更に公開されている RSA 公開鍵を **HMAC シークレットとして流用して**JWTを偽造することで、
認証バイパスを達成できてしまいます。
...
ここまで説明した古典的なアルゴリズム混同攻撃は、
「検証側が JWT ヘッダの `alg` をそのまま信用してしまう」実装を前提にしています。
一方で、今回 Hono で見つかった問題は、それと完全に同じ形ではありません。
...
このコードを書いた利用者が、
`publicKey` に RSA 公開鍵を渡して RS256 で検証しているつもりだったとしても、
...
JWKS エンドポイントのRSA公開鍵を HMAC シークレットとして署名してJWTを偽造することで、
古典的なアルゴリズム混同攻撃と同じメカニズムで認証バイパスが可能になってしまうことです。
...
sequenceDiagram
    ...
    Attacker->>JWKS: RSA公開鍵 (alg なし) を取得
...
RSA 公開鍵は 基本的にJWKSエンドポイントで公開されているため、
攻撃者が必要なものはその公開鍵と既存の `kid` だけで、特別な権限や内部情報は不要です。
そのため、攻撃者が攻撃を実行するためのハードルは非常に低いと言えます。

また、そもそもJWKSを利用するということは、
外部のIdP (Identity Provider) などから公開鍵を取得し、
それをもとにJWTを検証するケースという想定になると考えられます。
そのため、基本的に非対称鍵アルゴリズム (RS256 など) を利用するケースになります。

しかし当初の実装では、
JWKS 検証に対称鍵アルゴリズムである `HS256` / `HS384` / `HS512` を使うことを
明示的に拒否していませんでした。

JWKS は基本的に公開鍵を配布する仕組みとして使われるため、
そこに HMAC のような共有シークレット方式を混ぜられる設計は意図されていないはずです。
...
JWK/JWKS の場合は、対象が受け付ける既存の `kid` も重要になります。
特別な権限を必要とせず、
設定によってはネットワーク越しに認証・認可バイパスが成立し得ます。

**影響を受けるアプリケーションの条件**

- `hono/jwt` を使用し、`alg` を明示指定しておらず、非対称鍵で検証するつもりで公開鍵などを `secret` に渡している
- `hono/jwk` を使用し、JWKS のキーに `alg` フィールドが含まれておらず、JWT ヘッダの `alg` によって検証アルゴリズムが影響を受ける
...
JWK ミドルウェアで HS 系アルゴリズムを扱えること自体が混同の温床になります。
...
#### ③ JWK の `alg` と JWT ヘッダの `alg` の不一致を拒否する

JWK に `alg` フィールドが存在する場合は、
JWT ヘッダの `alg` と一致しているかを確認する案です。

修正前は `matchingKey.alg || header.alg` のように、
JWK 側の `alg` がなければヘッダの `alg` にフォールバックしていました。
修正後は、まずミドルウェアの allowlist で `header.alg` を検査し、
JWK 側に `alg` がある場合はそれとも一致することを確認します。

この対策は、JWK 側に明示されたアルゴリズムと
トークン側のアルゴリズムが食い違うケースを拒否するためのものです。
allowlist の代わりではなく、追加の整合性チェックとして位置づけています。

#### ④ JWK の `kty` や鍵パラメータからアルゴリズムを推測する
