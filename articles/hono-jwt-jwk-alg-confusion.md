...
対称鍵、公開鍵、秘密鍵のいずれも表現できますが、特に公開鍵を表現するために広く利用されています。
...
ここで、
「JWT を検証するだけなら 1 つのミドルウェアで十分なのではないでしょうか？」
と思う方もいるかもしれません。

しかし、実際には
**固定鍵で検証するケース** と**外部 JWKS から鍵を選んで検証するケース** では、
必要な入力も安全上の注意点も大きく異なります。

`hono/jwt` では、アプリケーションが検証鍵をどのように管理するかが中心になります。

一方、`hono/jwk` では、
JWK の `kid` や `alg` をどう扱うか、JWKS から取得した鍵をどこまで信用するか、
という論点が加わります。

今回の脆弱性は、
まさにこの **「アルゴリズムをどこから受け取り、どこまで信用するのか」**
という境界の扱いに問題があった、と言えます。
...
JWT検証を行う際の鉄則として、**JWT ヘッダの `alg` フィールドを信用してはいけない**という決まりがあります。
...
そもそもJWT検証にJWKSを利用するということは、
外部のIdP (Identity Provider) などから公開鍵を取得し、
それをもとにJWTを検証するケースが多いと思われます。

その場合、JWTの鍵として対称鍵を一般公開することは考えづらく、
基本的に非対称鍵アルゴリズム (RS256 など) を利用するケースになります。
