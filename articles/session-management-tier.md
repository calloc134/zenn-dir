...
- API サーバは JSON を受け取り JSON を返すことを前提
  - そのため、API は HTML フォーム送信を受け付けないものとする
...
- 4 位: B on β
  ...
  - 具体的なライブラリ
    ...
    - 以下のような対応
      ...
      - リフレッシュトークンの方は Web Worker のメモリ内に保存されるため 悪意ある JavaScript からのアクセスが比較的困難
...
- 先程のランキングの強さを比較する
  - 1 位 → 2 位:
    ...
    - 短命 assertion 型トークンが 盗まれた場合、
      - API 側で 失効させることができず、永続的アクセスのリスクがある という欠点が増える
      - ただし、短命であるため リスクは できるだけ低減される
  - 2 位 → 3 位:
    - 短命 assertion 型トークン + 長命 handle 型トークン on httpOnly クッキー から
    - assertion 型トークン on httpOnly クッキー へ
    - assertion 型トークンが 盗まれた場合、
      - API 側で 失効させることができず、永続的アクセスのリスクがある という欠点が増える
      - assertion 型トークンは 長命であるため リスクが高い
    - また、assertion 型 奪取による 永続的アクセスのリスクは 2 位と 変わらない
  - 3 位 → 4 位:
    ...
    - 適切な CORS (Cross-Origin Resource Sharing) 設定による SOP (Same-Origin Policy) の厳格な適用
      ...
      - CORS とは
        ...
        - SOP の制限を緩和するためのルールが CORS
          - Web 技術の発展により、別オリジン間での API アクセスの需要が高まったため
