...
- 以下の防御策を適用すれば ほとんどの CSRF 攻撃を防げる
  ...
  - API 全体に対して
    ...
    - 厳密な CORS (Cross-Origin Resource Sharing) 設定
      - `hono/cors` ミドルウェアを利用
      - ポリシーにより 被害者のブラウザにおける fetch API の動作に制限をかける
      - CORS ポリシーとは
        - 開いているサイトの JavaScript が fetch API などを用いて
          - 別ドメインの API にアクセスする場合の 制限ルール
        - ここでは 簡単のために オリジン = ドメイン だと思ってもらえば良い
        - ブラウザには SOP (同一生成元ポリシー) という セキュリティ機能があり
          - 開いているサイトの JavaScript で fetch API などを用いて
          - 別ドメインの API にアクセスする行為には 制限がかかっている
          - この SOP の制限を緩和するためのルールが CORS ポリシー
      ...
      - CORS 設定を適用した際の動作
        ...
        - 余談: HTML フォーム送信の場合は 例外があるが ここでは解説しない
